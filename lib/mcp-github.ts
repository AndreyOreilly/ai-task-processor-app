...
export class MCPGitHubService {
  ...
  constructor() {
    // MCP GitHub сервер будет запускаться отдельно
    this.client = null
  }
  ...
  async connect() {
    ...
    try {
      ...
      console.log('✅ MCP GitHub сервер подключен')
    } catch (error) {
      console.error('❌ Ошибка подключения к MCP GitHub:', error)
  ...
  async createPullRequest(data: {
    ...
    try {
      ...
      // Используем MCP для создания файла в репозитории
...
`
  ...
  async disconnect() {
    if (this.client && this.isConnected) {
      ...
      console.log('🔌 MCP GitHub сервер отключен')
    }
  }
} 